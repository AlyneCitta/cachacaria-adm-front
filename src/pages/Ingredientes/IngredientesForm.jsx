...
const ProdutoForm = () => {
  ...
  useEffect(() => {
    const fetchAuxiliares = async () => {
      try {
        const [catResponse, unidResponse, itensCompResponse] = await Promise.all([
...
console.log(payload)
  ...
  const goToProdutos = () => navigate('/ingredientes');
  ...
  return (
    <>
      ...
      <main>
        <BreadcrumbWrapper>
          <Breadcrumb>
            <span onClick={goToHome}>Principal</span> &gt; <span onClick={goToProdutos}>Ingredientes</span> &gt; <span>{id ? ' Editar ingrediente' : ' Incluir ingrediente'}</span>
          </Breadcrumb>
        </BreadcrumbWrapper>
        <PageWrapper>
          <PageContainer>
